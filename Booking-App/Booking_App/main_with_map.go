...
func getFirstNames() []string {
	firstNames := []string{}
	for _, booking := range bookings { //To only show value or index, we can omit the other output using an underscore (_)
...
func bookTickets(user_tickets uint, first_name string, last_name string, email string) {
	...
	var userData = make(map[string]string) // create a empty map
	userData["firstName"] = first_name
	userData["lastName"] = last_name
	userData["email"] = email
	userData["numberOfTickets"] = strconv.FormatUint(uint64(user_tickets), 10)
